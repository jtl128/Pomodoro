...
typedef struct {
    char reason[20];     // 打断原因
    int pomodoro_count;  // 在第几个番茄钟打断
    char time_str[20];   // 发生时间
} InterruptRecord;

// 每日统计结构体
typedef struct {
    char date[11];           // 日期 YYYY-MM-DD
    int total_pomodoros;     // 完成的番茄钟数
    int interrupt_count;     // 打断次数
    InterruptRecord interrupts[50];  // 打断记录
    int interrupt_index;      // 当前记录数
    char best_time[20];       // 最佳专注时段
    int focus_score;          // 专注力评分 0-100
} DailyStats;

// 全局变量
DailyStats today_stats;
int current_pomodoro = 0;     // 当前第几个番茄钟
int focus_minutes = 0;        // 今日总专注分钟数
...
// 保存数据到文件
void save_to_file() {
    FILE *file = fopen("pomodoro_data.txt", "a+");
    int i;
    ...
    fprintf(file, "打断次数:%d\n", today_stats.interrupt_count);
    fprintf(file, "专注分数:%d\n", today_stats.focus_score);
    fprintf(file, "最佳时段:%s\n", today_stats.best_time);
    
    // 写入打断记录
    for(i = 0; i < today_stats.interrupt_index; i++) {
        fprintf(file, "打断:%s|第%d个番茄钟|%s\n", 
                today_stats.interrupts[i].reason,
                today_stats.interrupts[i].pomodoro_count,
                today_stats.interrupts[i].time_str);
    }
...
// 加载历史数据
void load_from_file() {
    FILE *file = fopen("pomodoro_data.txt", "r");
    char line[200];
    
    if(file == NULL) {
        printf("🌟 首次运行，欢迎使用拖延症终结者！\n");
        return;
    }
